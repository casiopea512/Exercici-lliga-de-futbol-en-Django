...
class Partit(models.Model):
    ...
    #función para saber los goles
    def gols_locals(self):
        #self.event_set # es el mateix que 'Event.objects.filter(partit=self)' -> es la clase Event
        return self.event_set.filter(jugador__equip=self.equip_local,  tipus_esdeveniment="gol").count()
        #el '__' es igual que un join y filtres events que el jugador del (join) equip es igual al que está mostrant
    
    def gols_visitants(self):
